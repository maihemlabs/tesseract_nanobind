...
- **`tesseract_ros2_rosutils` bindings** (same opt-in flag) — wraps a high-leverage subset of the `tesseract_rosutils` conversion helpers from [`tesseract_ros2`][tesseract_ros2]: `Pose` ↔ `Eigen::Isometry3d`, `tesseract_msgs::EnvironmentCommand` ↔ `tesseract_environment::Command` (polymorphic dispatch + recursion through `Link` / `Joint` / `SceneGraph`), and both `tesseract_msgs::JointTrajectory` and `trajectory_msgs::JointTrajectory` ↔ `tesseract_common::JointTrajectory`. Trivial field-copy converters (`JointState` / `StringDoublePair[]` ↔ `unordered_map`) are intentionally not bound; they are shorter to write directly in pure Python against `rclpy`. Bound msg types are nanobind classes — **not** `rclpy` classes — so downstream code that publishes to ROS still copies fields between the bound msg and `rclpy.msg` of the same shape (mirrors the `Eigen::Isometry3d` ↔ numpy boundary already in use).
  - Adds `tesseract_common::JointTrajectory` to the existing `tesseract_common` module (a small prerequisite — only `JointState` was previously bound).
