...
- **`CartPosConstraint`'s per-axis coefficients + bounds constructor bound** — only the equality-form constructor was wired, so from Python the constraint was an equality on all six axes at unit weight. That cannot express either half of a process constraint: a **free axis** (a rotation the task is indifferent to — about an axis-symmetric tool, say — which must not be pulled back to the target) or an **asymmetric bound** (a standoff that may open but never close). Both are prerequisites for using the term as a COST via `IfoptProblem.addCostSet` rather than a hard pin; as an equality it only ever pulls straight back to the target pose, so a soft Cartesian preference was unexpressible and the free axis had to be given up entirely. The second C++ ctor (`coeffs`, `std::vector<Bounds>`) is now an additive overload — existing callers are unaffected — with `toBounds(lower, upper)` already exposed to build the band. Semantics worth knowing: a **zero coefficient drops that axis' row** (6 rows become 5), and `KEEP_AS_IS` preserves an asymmetric range while `SPLIT_TO_TWO_INEQUALITIES` splits each range into two one-sided rows. Eigen arguments stay required, per the module's `std::bad_cast`-on-Eigen-defaults constraint.
